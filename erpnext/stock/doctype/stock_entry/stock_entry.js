...
frappe.ui.form.on('Stock Entry', {
	onload: function(frm){
		if(frm.doc.work_order){
			frm.set_query("item_code", "items", function(doc, cdt, cdn) {
				...
				if (in_list(["Material Consumption for Manufacture", "Material Transfer"], doc.purpose)) {
					var filters = {
					...
					return {
						query : "erpnext.controllers.queries.get_available_items_wip",
						filters: filters
					}
				}
			});
	...
	setup: function(frm) {
		...
		frm.set_query('target_warehouse_address', function() {
			return {
				filters: {
					// link_doctype: ["in",["Warehouse","Distibutor Warehouse"]],
					link_name: ["in",[frm.doc.to_warehouse,frm.doc.distributor_warehouse]]
				}
			}
		});
		...
		frappe.db.get_list('Warehouse',{ filters:{
			'warehouse_type': ['in', ['Production', 'Production Reserve']]}, fields:['name']}).then( wh_wip_reserve =>{
			wh_wip_reserve = wh_wip_reserve.map(o=>{return o.name})
			frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
				...
				if(!item.item_code) {
					...
					if(in_list(wh_wip_reserve, item.s_warehouse)){
