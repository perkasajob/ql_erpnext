...
class PurchaseOrder(BuyingController):
	...
	def validate_supplier(self):
		...
		any_supplier_restrict = frappe.db.sql("""SELECT COUNT(*) as found FROM `tabItem` WHERE name IN (%s) and supplier_restrict= 1""" % (', '.join(['%s']* len(self.items))), tuple([i.item_code for i in self.items]), as_dict=True)
		if any_supplier_restrict[0].found > 0:
