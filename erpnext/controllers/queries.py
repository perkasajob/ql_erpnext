...
@frappe.whitelist()
@frappe.validate_and_sanitize_search_inputs
def get_available_items_wip(doctype, txt, searchfield, start, page_len, filters):
	args = {
	...
	if args.get('work_order'):
		item_codes = frappe.db.sql("""select sed.item_code, sed.item_name
...
def get_batch_no(doctype, txt, searchfield, start, page_len, filters):
	...
	if args.get('work_order'):
		stock_entries = frappe.db.sql("""select se.name
			from `tabStock Entry` se
				INNER JOIN `tabStock Entry Detail` sed on sed.parent = se.name
			where se.work_order = %(work_order)s
			      and se.docstatus = 1
		...
		stock_entries = ",".join('"'+str(se[0] + '"') for se in stock_entries)
		...
		batch_nos = frappe.db.sql("""select sle.batch_no, round(sum(sle.actual_qty),2), sle.stock_uom,
			...
			where
				...
				and sle.voucher_no IN ({stock_entries})
